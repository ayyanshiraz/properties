...
export default async function CommercialDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    try {
      ...
      if (!isNaN(dbId)) {
        ...
        if (dbProp) {
          ...
          propertyData = { id: dbProp.id.toString(), title: dbProp.title, location: dbProp.location, priceStr: `PKR ` + dbProp.price, area: dbProp.area || dbProp.category || `Not Specified`, image: combinedMedia.length > 0 ? combinedMedia[0] : `/buy/1.webp`, images: combinedMedia.length > 0 ? combinedMedia : [`/buy/1.webp`], description: dbProp.description, propertyType: dbProp.category || `Commercial`, propertyStatus: dbProp.type, propertyId: `bizbuy-` + dbProp.id + `805972` };
        }
      }
    } catch (error) {
      console.error(error);
  ...
  if (!propertyData) {
    propertyData = { id: routeId, title: `Property Not Found`, location: `Unknown`, priceStr: `N/A`, area: `N/A`, image: `/buy/1.webp`, images: [`/buy/1.webp`], description: `This property is no longer available.`, propertyType: `Unknown`, propertyStatus: `Unavailable`, propertyId: `N/A` };
  }
