...
interface CoWorkingClientProps {
  properties: PropertyItem[];
}

export default function CoWorkingClient({ properties }: CoWorkingClientProps) {
  ...
  useEffect(() => {
    if (gridRef.current && gridRef.current.children) {
      gsap.fromTo(Array.from(gridRef.current.children),
        { opacity: 0, z: -250, rotationX: 60, y: 80, scale: 0.8 },
        { opacity: 1, z: 0, rotationX: 0, y: 0, scale: 1, duration: 1.5, stagger: { amount: 0.6, from: "edges" }, ease: "back.out(1.4)", transformPerspective: 1200 }
      );
    }
  }, [viewMode, sortOrder]);
  ...
  return (
    <main className="w-full min-h-screen bg-[#fafafa] flex flex-col pt-[70px] overflow-hidden">
      ...
      <section ref={heroRef} className="relative w-full h-[45vh] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0 bg-[url(/buy/buy1.webp)] bg-cover bg-center bg-no-repeat transform scale-110"></div>
        ...
        <div ref={heroTextRef} className="relative z-10 flex flex-col items-center justify-center w-full px-4 text-center">
          ...
          <p className="text-lg md:text-2xl text-gray-200 font-light drop-shadow-lg">
            Discover Premium Shared Office Environments
          </p>
        </div>
      </section>
      ...
      <section ref={contentRef} className="w-full max-w-[1400px] mx-auto px-4 md:px-8 py-12 flex flex-col gap-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-gray-200 pb-6">
          ...
          <div className="flex flex-col gap-2">
            <div className="text-sm text-gray-500 font-medium">
              ...
              <span className="text-gray-900 font-bold">Co-Working</span>
            </div>
            <h2 className="text-4xl font-black text-gray-900 tracking-tight">Co-Working Spaces</h2>
        ...
        {properties.length === 0 ? (
          <div className="w-full flex flex-col items-center justify-center py-20 bg-white rounded-2xl border border-gray-100 shadow-sm text-center">
            ...
            <h3 className="text-2xl font-black text-gray-900 mb-2">No Co-Working Spaces Found</h3>
            <p className="text-gray-500 font-medium max-w-md">There are no co-working properties listed at the moment. Please check back later or add your own.</p>
          </div>
        ) : (
          <div ref={gridRef} className={"w-full gap-8 " + (viewMode === "grid" ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3" : "flex flex-col")}>
            {sortedProperties.map((property) => (
              <Link 
                href={"/co-working/" + property.id} 
                key={property.id} 
              ...
              >
                ...
                <div className={"flex flex-col justify-between p-6 " + (viewMode === "grid" ? "w-full" : "w-[60%]")}>
                  ...
                  <div className={"flex items-center gap-4 text-sm text-white font-bold pt-5 mt-5 border-t border-white/20 " + (viewMode === "grid" ? "justify-start" : "justify-start gap-10")}>
