...
export default function HomeClient({ dbProperties = [] }: HomeClientProps) {
  ...
  const handleSearch = () => {
    ...
    // --- NAYI DATABASE PROPERTIES KI SEARCH CHECK ---
    if (trimmedLocation !== "") {
      const matchedProp = dbProperties.find((p) => {
        const matchesText = p.title.toLowerCase().includes(trimmedLocation) || p.location.toLowerCase().includes(trimmedLocation);
        const matchesPurpose = purpose === "rent" ? p.type === "For Rent" : p.type !== "For Rent";
        return matchesText && matchesPurpose;
      });

      if (matchedProp) {
        if (matchedProp.type === "For Rent") {
          router.push(`/rent/${matchedProp.id}`);
          return;
        } else if (matchedProp.type === "Co-Working Space") {
          router.push(`/co-working/${matchedProp.id}`);
          return;
        } else if (matchedProp.category === "Commercial") {
          router.push(`/buy/commercial/${matchedProp.id}`);
          return;
        } else {
          router.push(`/buy/homes/${matchedProp.id}`);
          return;
        }
      }
    }

    // --- PURANI HARDCODED LOGIC KI SEARCH ---
    if (trimmedLocation.includes("valencia")) {
      router.push("/rent/1006");
      return;
    }
    if (trimmedLocation.includes("34 marla")) {
      router.push("/buy/homes/2006");
      return;
    }
