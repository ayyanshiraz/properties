...
export default function AreaGuidesClient({ areas }: AreaGuidesClientProps) {
  ...
  return (
    <main className="w-full min-h-screen bg-gray-50 flex flex-col pt-[70px]">
      ...
      <section ref={heroRef} className="relative w-full h-[70vh] flex items-center justify-start overflow-hidden">
        ...
        <div ref={textRef} className="relative z-10 flex flex-col items-start justify-center w-full max-w-[1400px] mx-auto px-4 md:px-8">
          <h1 className="text-5xl md:text-7xl lg:text-8xl font-extrabold text-white tracking-tight mb-4 uppercase drop-shadow-2xl">
            LAHORE AREA <span className="text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-400">GUIDES</span>
          </h1>
         ...
         <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
           {areas.map((area, index) => (
             <Link 
               ...
               key={index} 
               className="group relative block h-[450px] rounded-2xl overflow-hidden cursor-pointer shadow-lg hover:shadow-2xl transition-all duration-500"
             >
               <div className="absolute inset-0 bg-gray-900">
                 <img src={area.image} alt={area.name} className="w-full h-full object-cover opacity-70 group-hover:scale-110 group-hover:opacity-90 transition-all duration-700 ease-in-out" />
               </div>
               <div className="absolute inset-0 bg-gradient-to-t from-[#013220]/90 via-black/40 to-transparent opacity-80 group-hover:opacity-100 transition-opacity duration-500"></div>
               
               <div className="absolute bottom-0 left-0 w-full p-6 flex flex-col transform translate-y-6 group-hover:translate-y-0 transition-transform duration-500">
                 ...
                 <div className="mt-4 flex items-center gap-2 text-white font-semibold text-xs tracking-widest uppercase opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-200">
                   Explore Area 
