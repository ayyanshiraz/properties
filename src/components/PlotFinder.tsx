...
export default function PlotFinder() {
  ...
  const markerData = [
    { top: "30%", left: "25%", label: "2 Kanal Commercial Building +2 more" },
    { top: "45%", left: "45%", label: "Commercial House Kothi +5 more" },
    { top: "60%", left: "35%", label: "Furnished 2 Bed Apartment +12 more" },
    { top: "50%", left: "70%", label: "Non Furnished Apartment +8 more" },
    { top: "75%", left: "55%", label: "Zameen Aurum +20 more" }
  ];
  ...
  useEffect(() => {
    ...
    if (markersRef.current && markersRef.current.children) {
      gsap.fromTo(
        ...
        { scale: 1, y: 0, opacity: 1, duration: 1, stagger: 0.1, delay: 1, ease: "elastic.out(1, 0.5)" }
      );
    }
  ...
  return (
    <div className="fixed inset-0 z-[99999] w-full h-screen overflow-hidden bg-gray-200 flex flex-col">
      ...
      <div ref={mapContainerRef} className="absolute inset-0 z-0">
        ...
        ></iframe>
        {/* Subtle overlay to enhance UI contrast */}
        <div className="absolute inset-0 bg-[#013220]/10 pointer-events-none"></div>
      </div>

      {/* 2. Floating Search Bar (HUD Interface) */}
      <div className="absolute top-0 right-0 z-20 w-full px-4 pt-6 md:px-8 flex justify-end pointer-events-none">
        <div ref={searchBarRef} className="bg-white p-4 rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.2)] w-full max-w-5xl flex flex-col md:flex-row gap-4 items-end pointer-events-auto border border-gray-100">
          ...
          <div className="flex flex-col w-full md:w-2/3">
            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1 ml-1">Search Location</label>
            <div className="relative flex items-center">
              ...
              <input 
                type="text" 
                placeholder="Search live map data in Lahore" 
      ...
      {/* 3. Floating Society Markers Container */}
      <div ref={markersRef} className="absolute inset-0 z-10 pointer-events-none">
        {markerData.map((marker, idx) => (
          <div key={idx} className="absolute" style={{ top: marker.top, left: marker.left }}>
            <div className="flex items-center bg-gray-800/95 backdrop-blur-sm text-white text-[11px] font-bold rounded-sm shadow-xl pointer-events-auto cursor-pointer hover:bg-gray-900 transition-colors transform -translate-x-1/2 -translate-y-1/2">
