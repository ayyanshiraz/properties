...
export default function HomesDetailClient({ propertyId: fallbackId }: BuyDetailClientProps) {
  const params = useParams();
  const routeId = (params?.id as string) || fallbackId || `2`;
  ...
  return (
    <main className={`w-full min-h-screen bg-[#fcfcfc] flex flex-col pt-[70px] overflow-x-hidden`}>
      ...
      <div className={`w-full h-[60vh] relative overflow-hidden flex items-end pb-12 cursor-pointer`} onClick={() => setLightboxIndex(heroIndex)}>
        {isVideo(galleryImages[heroIndex]) ? (
          <video ref={heroImgRef} src={galleryImages[heroIndex]} autoPlay muted loop playsInline className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-1000 ease-in-out`} />
        ) : (
        ...
        <div ref={contentRef} className={`w-full lg:w-[65%] flex flex-col gap-14`}>
          ...
          <div className={`flex flex-col gap-4`}>
            <div className={`text-sm text-gray-500 font-bold uppercase tracking-wider flex flex-wrap items-center gap-3`}>
              ...
              <svg className={`w-4 h-4 text-gray-400`} fill={`none`} stroke={`currentColor`} viewBox={`0 0 24 24`}><path strokeLinecap={`round`} strokeLinejoin={`round`} strokeWidth={2} d={`M9 5l7 7-7 7`}></path></svg>
              <Link href={`/buy/homes`} className={`hover:text-[#013220] transition-colors`}>Homes</Link>
              <svg className={`w-4 h-4 text-gray-400`} fill={`none`} stroke={`currentColor`} viewBox={`0 0 24 24`}><path strokeLinecap={`round`} strokeLinejoin={`round`} strokeWidth={2} d={`M9 5l7 7-7 7`}></path></svg>
          ...
          <div className={`flex flex-col gap-8 pb-20`}>
            ...
            <div className={`grid grid-cols-2 md:grid-cols-3 gap-4`}>
               {galleryImages.map((media, idx) => (
                 ...
                 >
                   {isVideo(media) ? (
                     <video src={media} muted loop playsInline className={`w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 ease-out`} />
                   ) : (
                     <img src={media} alt={`Gallery view`} className={`w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 ease-out`} />
                   )}
        ...
        <div className={`fixed inset-0 z-[999999] bg-black/95 flex items-center justify-center backdrop-blur-xl`}>
          ...
          <div className={`relative w-full max-w-7xl max-h-[85vh] px-4 flex flex-col items-center justify-center select-none`} onClick={(e) => e.stopPropagation()}>
             {isVideo(galleryImages[lightboxIndex]) ? (
               <video ref={modalImgRef} src={galleryImages[lightboxIndex]} controls autoPlay className={`max-w-full max-h-[80vh] shadow-[0_0_80px_rgba(0,0,0,0.8)] rounded-lg`} />
             ) : (
