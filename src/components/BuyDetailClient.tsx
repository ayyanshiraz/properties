...
export default function BuyDetailClient({ property }: { property: PropertyData }) {
          ...
          <div className={`flex flex-col gap-8`}>
            ...
            <div className={`bg-white border border-gray-100 shadow-[0_10px_30px_rgba(0,0,0,0.03)] p-8 rounded-2xl flex flex-col gap-6`}>
              ...
              <div className={`w-full h-[450px] bg-gray-100 rounded-xl overflow-hidden shadow-inner border border-gray-200 mt-2`}>
                 <iframe src={`https://maps.google.com/maps?q=${encodeURIComponent(property.location)}&t=&z=14&ie=UTF8&iwloc=&output=embed`} width={`100%`} height={`100%`} style={{ border: 0 }} allowFullScreen loading={`lazy`} referrerPolicy={`no-referrer-when-downgrade`} title={`Live Map`}></iframe>
              </div>
            </div>
          </div>
